...
function validateStep(stepID){
    ...
    switch(stepID){
        ...
        case 3:
            ...
            break;
        case 4:
            validatePackageSize($('#min-units').val(),$('#max-units').val(),$('#package-size').val());
...
function validateBookingStartDate(bstart,bend){
...
function validateBookingEndDate(bstart,bend){
    var bookingstart= new moment(bstart.val(),'DD/MM/YYYY');
    var bookingend= new moment(bend.val(),'DD/MM/YYYY');
...
function validatePackageSize(min,max,packageSize){
    if(packageSize != '') {
...
function validateTaxRate(element) {
    if(element.val()!=''){
        var taxTest = /^((0|[1-9]\d?)(\.\d{1,2})?|100(\.000?)?)$/;

        if(!taxTest.test(element.val())){
            errors.push('Tax rate must be a valid percentage between 0 and 100');
        }
    }
}