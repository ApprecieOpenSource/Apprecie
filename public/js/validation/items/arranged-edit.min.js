...
function validateStartDate(a,b,c){if(""!=a.val()||""!=b.val())if(/^(2[0-3]|1[0-9]|0[0-9]|[^0-9][0-9]):([0-5][0-9]|[0-9])$/.test(b.val())||errors.push("The event start time is not valid, 24 hour format is required e.g. 09:00 or 23:00"),b=new moment(a.val()+" "+b.val(),"DD/MM/YYYY HH:mm"),b.isValid()){var d=new moment;b.isBefore(d,"day")&&errors.push("Start date cannot be in the past")}else errors.push("You must provide a valid start date");""==c.val()&&""!=a.val()&&errors.push("If you are providing an event start date please also supply a valid event end date")}
function validateEndDate(a,b,c,d){if(""!=a.val()||""!=d.val())/^(2[0-3]|1[0-9]|0[0-9]|[^0-9][0-9]):([0-5][0-9]|[0-9])$/.test(d.val())||errors.push("The event end time is not valid, 24 hour format is required e.g. 09:00 or 23:00"),a=new moment(a.val()+" "+d.val(),"DD/MM/YYYY HH:mm"),b=new moment(b.val()+" "+c.val(),"DD/MM/YYYY HH:mm"),a.isValid()&&b.isValid()?a.isBefore(b,"minute")&&errors.push("End date cannot be before the start date"):errors.push("You must provide a valid starat and end date")}
function validateBookingStartDate(a,b,c){a=new moment(a.val(),"DD/MM/YYYY");var d=new moment(c.val(),"DD/MM/YYYY"),e=new moment(b.val(),"DD/MM/YYYY");a.isValid()?(""!=b.val()&&(a.isSame(e,"day")||a.isAfter(e,"day"))&&errors.push("The booking start date must be before the event start date"),""!=c.val()&&a.isAfter(d,"day")&&errors.push("The booking start date must be before the booking end date")):errors.push("You must provide a valid booking start date")}
function validateBookingEndDate(a,b,c){if(""!=c.val()){a=new moment(a.val(),"DD/MM/YYYY");c=new moment(c.val(),"DD/MM/YYYY");var d=new moment(b.val(),"DD/MM/YYYY");c.isValid()?(""!=b.val()&&(c.isSame(d,"day")||c.isAfter(d,"day"))&&errors.push("The booking end date must be before the event start date"),c.isBefore(a,"day")&&errors.push("The booking end date must be after the booking start date")):errors.push("You must provide a valid booking end date")}}
...
function validateMaxPlaces(a){isNaN(a)||""==a||1>a?errors.push("Number of Packages must be a whole number and greater than 0"):0!=a%1&&errors.push("Number of Packages must be a whole number")}function validatePricePerUnit(a){isNaN(a.val())?errors.push("The Price per Package must be a number"):0<a.val()&&.5>a.val()&&errors.push("The Price per Package must be 0 or higher than 0.50")}function validateCostPerUnit(a){""!=a.val()&&isNaN(a.val())&&errors.push("The Cost per Attendee must be a number")}
function validateCostToDeliver(a){""!=a.val()&&isNaN(a.val())&&errors.push("The Static Costs must be a number")}function validateMarketValue(a){""!=a.val()&&isNaN(a.val())&&errors.push("The Compliance Value must be a number")}function validateTaxRate(a){""!=a.val()&&(a=parseFloat(a.val()),(isNaN(a)||3<a.precision()||0>a||100<a)&&errors.push("Tax rate must be a valid percentage between 0 and 100 with a maximum precision of 3 decimal places"))};
