...
function createEvent(){
    loader(true);
    return $.ajax({
        url: "/itemcreation/ajaxCreateArranged",
        type: 'post',
        dataType: 'json',
        cache: false,
...
function validateEndDate(enddate,startdate,stime,etime){
    ...
    if(startdate.val() == '' && enddate.val() != '') {
        errors.push('If you are providing an event end date please also supply a valid event start date');
    }
}
...
function validatePricePerUnit(element){
    if (isNaN(element.val())) {
        ...
        errors.push('The Price per Package if provided at this stage, must be 0 or higher than 0.50');
    }
}
