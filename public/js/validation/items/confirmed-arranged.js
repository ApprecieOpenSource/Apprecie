...
function validateStep(stepID){
    ...
    switch(stepID){
        ...
        case 2:
            ...
            validateBookingEndDate($('#confirmed-startdate'),$('#confirmed-bookingend'));
            nextStep(2);
            break;
        case 3:
...
function editEvent(eventId){
    loader(true);
    return $.ajax({
        url: "/itemcreation/ajaxEditEvent/"+eventId,
        type: 'post',
        dataType: 'json',
        cache: false,
...
function validateBookingEndDate(estart,bend){
    var earliesbookingend = new moment(earliestBookingEndDate, 'DD/MM/YYYY');
    var bookingstart = new moment(bookingStartDate, 'DD/MM/YYYY');
    var bookingend = new moment(bend.val(), 'DD/MM/YYYY');
    var eventstart = new moment(estart.val(), 'DD/MM/YYYY');
    if(bookingend.isValid()){
        /*
        console.log('booking start:'+bookingstart.toString());
        console.log('booking end:'+bookingend.toString());
        console.log('event start:'+eventstart.toString());
        console.log('earlies booking end:'+earliesbookingend.toString());
        */
        ...
        if (bookingend.isBefore(earliesbookingend, 'day')) {
            errors.push('The booking end date must be ' + earliestBookingEndDate + ' at the earliest');
        }
    }
    else{
